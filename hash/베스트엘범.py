...
def solution(genres, plays):
    ...
    for i in play_t:
        order_g[i[0]] = sorted(order_g[i[0]], key=lambda x: x[0],
                               reverse=True)[:2]
...
'''
처음 시도했던 방법 answer에 추가하는 부분에서 입출력 예시 였던 genres의 리스트 길이가 길어지면
 list index out of range 가 나서 문제였던것.
def solution(genres, plays):
    answer = []
    play_time = {} # 총 플레이 수
    ...
    #  문제였던 부분
    # for i in play_time:
    #     order_g[i[0]] = sorted(order_g[i[0]],
